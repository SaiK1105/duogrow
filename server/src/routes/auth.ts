...
import { hashCredential, MIN_SECRET_LENGTH, verifyCredential } from "../lib/credentials.js";
...
async function readCredentials(c: Context<AppEnv>): Promise<{ name: string; secret: string }> {
...
authRoutes.post("/register", async (c) => {
  const { name, secret } = await readCredentials(c);
...
authRoutes.post("/login", async (c) => {
  ...
  if (!existing || !verifyCredential(secret, existing.credential_salt, existing.credential_hash)) {
