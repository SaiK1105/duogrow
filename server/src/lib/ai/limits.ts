...
export function reserveAiRequest(request: ReserveAiRequest): AiReservation {
  ...
  const reserve = db.transaction(() => {
    const userCost = (db.prepare("SELECT COALESCE(SUM(reserved_cost_cents), 0) AS total FROM ai_usage_daily WHERE user_id = ? AND date = ?").get(request.actorUserId, request.date) as { total: number }).total;
    ...
    if (request.feature === "duo_reflection") {
      ...
      const used = (db.prepare("SELECT COALESCE(SUM(request_count), 0) AS total FROM ai_usage_daily WHERE duo_id = ? AND feature = ? AND date BETWEEN ? AND ?").get(request.duoId, request.feature, weekStart(request.date), request.date) as { total: number }).total;
      if (used >= 1) throw new AiLimitError("feature");
    } else {
      const used = (db.prepare("SELECT request_count FROM ai_usage_daily WHERE user_id = ? AND feature = ? AND date = ?").get(request.actorUserId, request.feature, request.date) as { request_count: number } | undefined)?.request_count ?? 0;
    ...
    db.prepare(`INSERT INTO ai_usage_daily (user_id, duo_id, feature, date, request_count, reserved_cost_cents) VALUES (?, ?, ?, ?, 1, ?)
      ...
      .run(request.actorUserId, request.duoId ?? null, request.feature, request.date, request.estimatedCostCents);
  ...
  return {
    rollback(): void {
      if (!active) return;
      db.transaction(() => {
        db.prepare("UPDATE ai_usage_daily SET request_count = request_count - 1, reserved_cost_cents = reserved_cost_cents - ? WHERE user_id = ? AND feature = ? AND date = ?").run(request.estimatedCostCents, request.actorUserId, request.feature, request.date);
        db.prepare("DELETE FROM ai_usage_daily WHERE user_id = ? AND feature = ? AND date = ? AND request_count = 0").run(request.actorUserId, request.feature, request.date);
